...
GeneticAlgorithm.prototype = {
	...
	createPopulation: function() {
		...
		for (var i = 0; i < this.max_units; i++) {
			...
			var newUnit = new synaptic.Architect.Perceptron(2, 6, 4);
	...
	evolvePopulation: function() {
		...
		var PopulationCopy = this.Population
		var Winners = this.selection();

		if (this.mutateRate == 1 && Winners[0].fitness == 0) {
			this.createPopulation();
			alert("new population")
		} else if (Winners[0].fitness < this.bestFitness) {
			this.Population = PopulationCopy
			Winners = PopulationCopy.slice(0, this.top_units);
		} else {
			this.mutateRate = 0.2;
